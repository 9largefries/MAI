...
template <class T1, class T2, class T3>
ArrayItem<T1, T2, T3>::ArrayItem(std::shared_ptr<T2> &rhomb, int index) :
	pentagon(nullptr),
	rhomb(rhomb),
	trapeze(nullptr),
	index(index),
...
{
	if (item.IsPentagon()) 
		os << *item.pentagon << " Square = " << item.square << " (I am pentagon)";
	else if (item.IsRhomb()) 
		os << *item.rhomb << " Square = " << item.square << " (I am rhomb)";
	else if (item.IsTrapeze()) 
		os << *item.trapeze << " Square = " << item.square << " (I am trapeze)";
	else 
		os << "Empty";
	return os;
}
