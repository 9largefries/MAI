...
template <class T1, class T2, class T3> void FigureArray<T1, T2, T3>::DeleteBySquare(int square)
{
	for (int i = 0; i < size; i++)
	{
		if (data[i].square < square) Delete(i);
	}
}
...
template class FigureArray<Pentagon, Rhomb, Trapeze>;
template std::ostream& operator<<(std::ostream& os, FigureArray<Pentagon, Rhomb, Trapeze>&array);