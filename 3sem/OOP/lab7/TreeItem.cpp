...
template <class T1, class T2, class T3> TreeItem<T1, T2, T3>::TreeItem() : pentagon(nullptr), rhomb(nullptr), trapeze(nullptr)
{
	for (int i = 0; i < max_children_size; i++) children[i] = nullptr;
	square = 0;
}

template <class T1, class T2, class T3> TreeItem<T1, T2, T3>::TreeItem(std::shared_ptr<T1> &pentagon) : pentagon(pentagon), rhomb(nullptr), trapeze(nullptr)
{
	for (int i = 0; i < max_children_size; i++) children[i] = nullptr;
	square = pentagon->Square();
}

template <class T1, class T2, class T3> TreeItem<T1, T2, T3>::TreeItem(std::shared_ptr<T2> &rhomb) : pentagon(nullptr), rhomb(rhomb), trapeze(nullptr)
{
	for (int i = 0; i < max_children_size; i++) children[i] = nullptr;
	square = rhomb->Square();
}

template <class T1, class T2, class T3> TreeItem<T1, T2, T3>::TreeItem(std::shared_ptr<T3> &trapeze) : pentagon(nullptr), rhomb(nullptr), trapeze(trapeze)
{
	for (int i = 0; i < max_children_size; i++) children[i] = nullptr;
...
template <class T1, class T2, class T3> bool TreeItem<T1, T2, T3>::IsRhomb()
{
...
template <class T1, class T2, class T3> std::shared_ptr<T2> TreeItem<T1, T2, T3>::GetRhomb()
{
	return this->rhomb;
}
...
template <class T1, class T2, class T3> void TreeItem<T1, T2, T3>::SetChild(std::shared_ptr<T2> &rhomb, int index)
{
	children[index] = new TreeItem<T1, T2, T3>(rhomb);
}
...
template <class T1, class T2, class T3> std::ostream& operator << (std::ostream &os, TreeItem<T1, T2, T3> item)
{
	...
	else if (item.IsRhomb()) os << *item.rhomb << " Square = " << item.square << " (I am rhomb)";
